...
import {
  ...
  QuestionService,
  LearningService,
  continueModuleLearningAction,
  continueSectionLearningAction,
  learnFalseQuestionsFromModuleAction,
  signOutAction
} from "core";
import { Fonts } from "../utils/Fonts";

const OneModule = ({exam, modules, mid}) => (
  <StatisticsCategory
    hideButton
...
class StatisticScene extends Component {
  ...
  render() {
    ...
    var seenQuestions = keys.reduce(
      (pv, key) => pv + (mods[key].seenQuestions || 0),
      0
    );
    var questionCount = keys.reduce(
      (pv, key) => pv + (mods[key].questionCount || 0),
      0
    );
    var falseQuestions = keys.reduce(
      (pv, key) => pv + (mods[key].falseQuestions || 0),
      0
    );
    var lernState = seenQuestions / questionCount;
    const successRate =
      keys.reduce((pv, key) => pv + (mods[key].successRate || 0), 0) /
      keys.length;
    return (
      <View style={{ flex: 1 }}>
        <SafeAreaView style={{ backgroundColor: "#003A65" }}>
          <StatusBar backgroundColor="#003A65" barStyle="light-content" />
        </SafeAreaView>
        <MainHeader
          text="Weiter lernen"
          style={{
          ...
          children2={
            <Image
              ...
              source={require("../img/ic_options.png")}
            />
          }
          optionsPress={() => this.toogleModal()}
        />
        <ScrollView style={styles.containerStyle} resizeMode="cover">
          <SafeAreaView>
            <Text style={styles.titleTextStyle}>Allgemein</Text>
            <View style={{ marginTop: 12 }}>
              ...
              >
                <Progress.Bar
                  ...
                  style={{ width: "82%" }}
                  color={"#58D980"}
                  unfilledColor="rgba(0, 58, 101, 0.2)"
                  borderWidth={0}
                  borderRadius={0}
                />
                <Text style={styles.percentTextStyle}>
                  {(lernState * 100).toFixed(0)}%
                </Text>
              </View>
            </View>
            <View style={{ marginTop: 12 }}>
              <Text style={styles.statisticTextStyle}>Erfolgschance</Text>
              <View
                style={{
                  marginLeft: 20,
                  marginRight: 20,
            ...
            <Text style={styles.statisticTextStyle}>Prüfungshistorie</Text>
  ...
  mapDateTimeToNiceString(date) {
    ...
    if (diffDate.getFullYear() == 1970) {
      if (diffDate.getMonth() == 0) {
        if (diffDate.getDate() == 1) return "Heute";
        if (diffDate.getDate() == 2) return "Gestern";
        if (diffDate.getDate() == 3) return "Vorgestern";
        ...
        if (diffDate.getDate() < 9) return `Vor einer Woche`;
        if (diffDate.getDate() < 9 + 7) return "Vor zwei Wochen";
      }
    }
  ...
  mapStatisticCategories() {
    ...
    return Object.keys(exams)
      ...
      .map(key => {
        ...
        lastDate = currDate;
        var justOneModule = Object.keys(exams[key].exam).length == 1;
        return (
          <View>
            {isDifferent && (
              <Text style={styles.dateStyle}>
                {this.mapDateTimeToNiceString(new Date(parseInt(key)))}
              </Text>
            )}
            {justOneModule && <OneModule key={key} exam={exams[key]} modules={this.props.modules.modules}  mid={Object.keys(exams[key].exam)[0]}/>}
            {justOneModule || (
              <ExamStatistic
                key={key}
                exam={exams[key]}
  ...
  startSectionLearning() {
    this.props.dispatchContinueSectionLearning(
      this.props.modules.currentModuleID
    );
  }
  startLearning() {
    this.props.dispatchContinueModuleLearning(
      this.props.modules.selectedSubmodule
    );
  }
}
